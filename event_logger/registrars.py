...
def create_log_record(request=None, log_user=None, log_detail=None, log_target=None):
    """This function takes arguments and process them and send them to LogRecordsModel create method to create
       ...
       The first argument is request=None, Send the request object here.
       The second argument is log_user=None, send the user object here.
       The third argument is log_detail=None, send a nice event description text message here.
       The fourth argument is log_target=None, send that object on which that action was performed. As for
       example the User visited a detail page of a Book Object. In that case, you may call this function with
       these values.
       create_log_records(request, request.user, 'read operation', book.object)
       This function will return the created object if successful"""
    log_record_object = LogRecordsModel(log_user=log_user, log_detail=log_detail, log_target=log_target, event_path=get_request_event_path(request))
